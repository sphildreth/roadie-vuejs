    ...
    <v-container v-if="loaded" fluid grid-list-md class="artist-edit-container">
      <v-layout row class="ma-1 mb-3">
        ...
        <v-flex xs11>
          ...
          <div class="caption ml-2">Allowed JPG, GIF or PNG. Image will be converted to JPG and Artist thumbnail will be resized to 320x320 pixels.</div>
        </v-flex>
      </v-layout>      
      <v-layout row>
        <v-flex xs4>
...
<script>
  ...
  export default {
    ...
    methods: {      
      resetImage() {
        this.imageUrl = this.artist.mediumThumbnail.url + '?ts=' + new Date().getTime();
      },
      changeAvatar() {
      ...
      updateData: async function() {
        ...
        .get(process.env.VUE_APP_API_URL + `/artists/${this.id}`)
          .then(rr => {
            ...
            // ▟
            this.artist.birthDate = this.artist.birthDate ?  new Date(this.artist.birthDate).toISOString().substr(0, 10)  : null;           
            this.artist.beginDate = this.artist.beginDate ?  new Date(this.artist.beginDate).toISOString().substr(0, 10)  : null;
            this.artist.endDate = this.artist.endDate ?  new Date(this.artist.endDate).toISOString().substr(0, 10)  : null;            
            this.imageUrl = this.artist.mediumThumbnail.url + '?ts=' + new Date().getTime();
      ...
      doArtistSearch: function (val) {
        ...
        .then(res => {         
          res.data.rows.forEach((a) => {
            ...
            if(!this.$_.find(this.lookupData.artistItems, function(l) { return l.value === artist.value;})) {
